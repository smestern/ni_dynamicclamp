...
def attach_neuron(neurongroup, idx=0, v_mem_var='v', i_mem_var='I_in'):
    ...
    neuron = neurongroup[idx:idx+1]
    #add a run regularly statement to the neuron group
    neuron.run_regularly(f'{v_mem_var} = step_clamp(t, {i_mem_var})', dt=defaultclock.dt, when='end')
    #update the resetter of the full group to exclude the neuron we are attaching
    # for key, value in neurongroup.event_codes.items():
    #     if key == 'spike': #here we want to update the event reset
    #         #assuming the value is a list of reset statements seperated by semicolons
    #         new_value = ''
    #         for v in value.split(';'):
    #             if v.strip() != '':
    #                 new_value += f'{v}*int(i == {idx});' #new reset condition avoids idx of real neuron
    #         neurongroup.event_codes[key] = new_value
